...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _saveButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"保存到相册  ", nil) style:UIBarButtonItemStylePlain target:self action:@selector(saveButtonPressed:)];
        
        self.navigationItem.rightBarButtonItems = @[_doneButton,_saveButton];
    } else {
...
- (void)saveButtonPressed:(id)sender {
    if (_saveButton) {
        ...
        if ([_delegate respondsToSelector:@selector(savePhotoToPhotoAlubmWithPhotoBrowser:photoAtIndex:)]) {
            // Call delegate method and let them dismiss us
            [_delegate savePhotoToPhotoAlubmWithPhotoBrowser:self photoAtIndex:_currentPageIndex];
        } else  {
        }
    }
}
